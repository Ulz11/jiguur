...
/* ═══════════════════════════════════════════════════════════════════════
   ⚠ ПИН — ЭЗЭНИЙ ШИЙДВЭР ХҮЛЭЭЖ БАЙНА (тест ОДООГИЙН зан төлвийг барина)

   Мөнгөний хана нь гэрээний дэлгэрэнгүй дээр бүтэн: сервер өөрөө талбарыг
   хасдаг. Гэвч ЦЭСЭНД даргад «Бартер» хуудас нээлттэй байна
   (`App.tsx`-ийн NAV дээр `hide: "factory"` алга) бөгөөд:
     · `GET /api/barter` нь ЯМАР Ч рольд нээлттэй (`auth.current_user`);
     · `pages/Barter.tsx` нь хадгалагдаж буй хөрөнгийн НИЙТ ҮНЭ, зарсан дүн,
       ОЛСОН АШИГ, мөр бүрийн орсон үнэ/санал болгож буй үнэ/зарсан дүнг
       бүтэн ₮-өөр зурна (зөвхөн ЗАРАХ товч нь `canSell`-ээр хаагдсан).
   Өөрөөр хэлбэл үйлдвэрийн дарга компанийн бартерын хөрөнгийн мөнгийг
   планшет дээрээ бүтнээр хардаг.

   ЯАГААД ЭНД ЗАСААГҮЙ ВЭ: түүнд энэ хуудас ХЭРЭГТЭЙ — бартераар орж ирсэн
   хөрөнгийг агуулахад тооцох (`POST /barter/{id}/to-stock`) нь ЗӨВХӨН
   manager/factory-д нээлттэй, өөрөөр хэлбэл түүний ажил. Тиймээс «хуудсыг
   хаах» нь буруу хариулт; зөв хариулт нь «ямар тоо түүнд хэрэгтэй вэ»
   гэсэн ЭЗЭНИЙ шийдвэр (тоо ширхэг, төлөв — тийм; үнэ, ашиг — үгүй?).
   UI-ЗАРЧИМ §1-ийн «даргад хаалттай» жагсаалтад Бартер ОРООГҮЙ тул энэ нь
   бичигдсэн гэрээний зөрчил ч биш.

   Энэ тест УНАВАЛ: хана нэмэгдсэн гэсэн үг — тэр үед ЭНЭ тестийг устгаж,
   дээрх `MONEY_KEYS`-тэй ижил хэлбэрийн ЖИНХЭНЭ ханын тест бичнэ.
   ═══════════════════════════════════════════════════════════════════════ */
test('ПИН: дарга Бартер хуудсан дээр компанийн ₮-г ХАРСААР байна (шийдвэр хүлээж буй)',
  async ({ factoryPage }) => {
    await factoryPage.goto('/barter');
    await expect(factoryPage.getByRole('heading', { level: 1 })).toContainText('Бартер');

    const seen = await factoryPage.evaluate(() => {
      const out: string[] = [];
    ...
    expect(seen.length,
      'Бартер дээр ₮ алга — хана нэмэгдсэн бол ЭНЭ ПИН-ийг устгаж, ' +
      'жинхэнэ ханын тест бич').toBeGreaterThan(0);

    /* Цэсэнд нь ч байгааг пиннэ — хоёулаа нэг шийдвэрийн хоёр тал. */
