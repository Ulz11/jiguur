 ...
 *
 *  ГЭРЭЭНИЙ хуудсан дээр «хэн» нь мэдэгдэж байдаг (толгой нь хэлсэн) тул
 *  `who` хоосон. ДАШБОАРД дээр 6 гэрээний ачилт зэрэгцэж зогсдог: аль нь
 *  баталгаажсаныг нэрлээгүй зурвас нь «дараад юу ч болсонгүй»-тэй адил.
 *  Тоо мэдэгдэхгүй бол (мөрүүд нь татагдаагүй) мөрөнд «0ш» гэж ХУДАЛ
 *  бичихгүй — тэр хэсэг нь өөрөө унана. */
export function shipmentOutcome(f: {
  qty?: number | null; date: string; who?: string; movementId?: number | null;
}): Outcome {
  return {
    text: line("Ачилт баталгаажлаа",
               [f.who?.trim(), f.qty ? `${fmt(f.qty)}ш` : "",
                `${f.date}-наас тооцоонд орлоо`]),
    mark: freshMark("mv", f.movementId),
  };
}
