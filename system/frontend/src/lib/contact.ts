...
/* ---------- ХҮНИЙ ТУХАЙ ТЭМДЭГЛЭЛ (№72, 73) ----------
 *
 * `note` талбар нь БИЧИГДДЭГ атлаа ХЭЗЭЭ Ч харагддаггүй байв. Тэр нь хоёр
 * шалтгаантай: (1) шилжүүлэг түүнд Excel-ийн НҮДНИЙ ХАЯГ хадгалсан
 * («БЛҮҮМ-2!O39») — Отгоо эгч нэрийнхээ доор ийм юм харах ёсгүй; (2) карт нь
 * бүхэлдээ мөрөнд багтахгүй байв.
 *
 * Хоёр асуудлыг НЭГ дүрмээр шийднэ: машины хаяг НУУГДАНА, хүний бичсэн
 * тэмдэглэл («тооцоо нийлдэг», «амралттай») нэрийнхээ доор ГАРНА. Талбар нь
 * өгөгдөл дээрээ БҮТНЭЭРЭЭ үлдэнэ (устгал байхгүй) — зөвхөн харагдац.
 */

/** Excel-ийн нүдний хаяг уу («БЛҮҮМ-2!O39», «2026 тооцоо!R24»). */
export function isSheetRef(note: string | null | undefined): boolean {
  return /![A-ZА-ЯӨҮ]{1,3}\d{1,5}\b/i.test((note || "").trim());
}

/** Дэлгэц дээр гарах тэмдэглэл — машины хаяг бол ХООСОН. */
export function contactNote(note: string | null | undefined): string {
  const s = (note || "").trim();
  return isSheetRef(s) ? "" : s;
}
