...
Гурван дүрэм:
  ...
  · **Мөр бүр өдөрт нэг** — гүйлт бүр Audit-д НЭГ мөр үлдээнэ, юу ч
    үүсээгүй бол ч. Урьд нь «чимээгүй байдал» гэсэн дүрэм байсан: 0 үүссэн
    өдөр юу ч бичигдэхгүй. Гэвч тэр нь хамгийн чухал асуултыг хариулагдахгүй
    үлдээдэг — «Өчигдөр гүйлт ҮНЭХЭЭР явсан уу, эсвэл сервер унтарсан уу?»
    Отгоо эгчийн хувьд хоосон мөр байхгүй нь «бүх юм хэвийн» ГЭСЭН ҮГ БИШ,
    «мэдэхгүй» гэсэн үг. Одоо «0 шинэ нэхэмжлэл» гэдэг нь БАРИМТ болж үлдэнэ.
"""
import asyncio
import os
...
def run_line(res: dict) -> str:
    """Гүйлтийн НЭГ ӨГҮҮЛБЭР — cron ба гар товчлуур ХОЁУЛАА үүнийг бичнэ.

    ⚠ Мөрийн ЭХЭНД «cron:» гэж бичигддэг байв — /audit-ийн «Дэлгэрэнгүй»
    багана нь Отгоо эгчийн НҮД, тэнд англи үг байх ёсгүй. Серверийн лог
    (`print`) нь хөгжүүлэгчийнх тул «[cron]» хэвээр.

    АЛДАА нь мөрөнд ОРНО. Урьд нь зөвхөн `print` рүү явдаг байсан: сервер
    дахин асахад тэр мөр алга болж, «яагаад энэ гэрээ нэхэгдээгүй юм бэ?»
    гэсэн асуулт хариултгүй үлддэг. Техникийн текст (алдааны төрөл) нь
    монгол өгүүлбэрийн ДОТОР, хаалтанд явна — уншигч ядаж ХЭДЭН гэрээ,
    АЛЬ гэрээ гэдгийг мэднэ.
    """
    line = f"Өдөр тутмын гүйлт: {res['created']} шинэ нэхэмжлэл"
    if res["contracts"]:
        line += (f" · {len(res['contracts'])} гэрээ "
                 f"({', '.join(res['contracts'])})")
    line += f" · {res['date']}"
    if res["errors"]:
        bad = " | ".join(f"№{e['no']}: {e['error']}" for e in res["errors"])
        line += f" · ⚠ {len(res['errors'])} гэрээ нэхэгдсэнгүй: {bad}"
    return line
