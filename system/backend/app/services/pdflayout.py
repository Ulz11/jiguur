...
MUTED = (115, 128, 140)
LINE = (222, 227, 232)
# Хүснэгтийн нүдний тор — Excel-ийн «All Borders» мэдрэмж: тод боловч цэвэрхэн.
# LINE (зөөлөн section тусгаарлагч) нь бүдэг тул хүснэгтэд ашиглавал тор бараг
# үл үзэгдэнэ; GRID нь илт бараан (харанхуй саарал) тул дөрвөн баримт бүгд ижил,
# тод харагдах тортой болно. Зузаан нь ~0.5pt — border мэт нарийн боловч тод.
GRID = (90, 96, 105)
GRID_W = 0.5
...
def vline(doc: Doc, x: float, y0: float, y1: float) -> None:
    """`x` дээр `y0`-оос `y1` хүртэл БОСОО зураас (GRID өнгө, GRID_W зузаан).
    ...
    зузааныг дуудлага бүрт дахин тавьна (баримтын хуваалцсан төлөв). Хүснэгтийн
    тор тул зөөлөн LINE биш, ТОД GRID-ээр — pdfgen-ийн border-тай ижил тон."""
    doc.pdf.set_draw_color(*GRID)
    doc.pdf.set_line_width(GRID_W)
...
def cell_row(doc: Doc, xs: list[float], y0: float, y1: float) -> None:
    ...
    Тор нь зөөлөн LINE биш, ТОД GRID-ээр (GRID_W зузаан) татагдана — дөрвөн
    баримтын хүснэгт бүгд pdfgen-ийн border-тай нэг тонд орж, жигд харагдана."""
    doc.pdf.set_draw_color(*GRID)
    doc.pdf.set_line_width(GRID_W)
