...
def _payment_clauses(c: models.Contract, day_total: float, sale_total: float) -> list[str]:
    ...
    elif billing.cycle_mode(c) == "month":
        # КАЛЕНДАРЬ САР: гэрээнд «30 хоног тутам» гэж хэвлэвэл гарын үсэгтэй
        # баримт нь системийн тооцоотой зөрнө — 31 хоногтой сар ×31/30 нэхэгдэнэ.
        lines += [
            f"2.1. Түрээсийн төлбөрийг сар бүрийн {c.start_date.day}-нд тооцож "
            f"нэхэмжлэнэ. Тооцооны хоног нь тухайн сарын жинхэнэ урттай тэнцүү "
            f"(28–31 хоног); нэг сарын дүн ойролцоогоор "
            f"{_money(day_total * 30)}–{_money(day_total * 31)}.",
